...
import { getOneQuestion, addAnswer, likeAnswer, markSolved } from '../../actions/question'
...
class QuestionPage extends Component {
  ...
  render() {
    ...
    return (
      <DocumentTitle title={ singleQuestion.title ? singleQuestion.title : 'GeekAsks' }>
        <div id="questionpage">
          <Header />
              ...
              <div className="container page">
                <div className="row-fluid">
                  <div className="question-author">
                    ...
                    <span className="question-author__email text text--gray text--xs">{`#${singleQuestion.author.email}`}</span>
                  </div>
                </div>
                <div className="row-fluid">
                  <ul className="selected-tags">
                    {
                      singleQuestion.tags && singleQuestion.tags.map((tag, index) => {
                        return (
                          <li key={ index } className="selected-tags__item">
                            <img src={require(`../../uploads/tags/${tag.cover}`)} alt="tag"/>
                            <span>{ tag.title }</span>
                          </li>
                        )
                      })
                ...
                <div className="row-fluid">
                  <h3 className="title question-title title--sm">{singleQuestion.title}</h3>
                </div>
                <div className="row-fluid">
                  <div className="question-body">
                    {singleQuestion.body && _parseString(singleQuestion.body).map((el, index) => {
                      switch (el.tagName) {
                        case 'CODE':
                ...
                <div className="answers-container">
                  { singleQuestion.answers &&
                    <div className="row-fluid">
                      <h3 className="title title--xs">Ответы на вопрос ({ singleQuestion.answers ? singleQuestion.answers.length : 0 })</h3>

                      <div className="question-answers">
                        { singleQuestion.answers.map((item, index) => {
                          return (
                            <QuestionAnswer likeAnswer={likeAnswer}
                                            item={item}
                                            key={index}
                                            answerId={item._id}
                                            userName={userName}
                  ...
                  { authenticated ?
                     <div className="row-fluid">
                        <div className="new-comment">
                          ...
                          <Button className="new-comment__submit"
                                  type="success"
                                  onClick={submitAnswer}
                                  size="small">Опубликовать</Button>
                        </div>
                    </div>
                    :
                    <div className="box box-center answer-login">
                      <p className="text text--xs">
                        <Link to="/login" className="el-button el-button--success el-button--small">Войдите</Link> чтоб ответить на вопрос
                      </p>
                    </div>
                  }
                </div>
