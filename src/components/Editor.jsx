...
class Editor extends Component {
  ...
  _parseString(str) {
    ...
    return _.map(cleanParser.parseFromString(res, 'text/html').body.children, (el) => el)
  }
  ...
  render() {
    ...
    return (
      <div>
        {/* PREVIEW */}
        <div className="editor-preview" style={{ display: preview ? 'block' : 'none' }}>
            ...
            switch (el.tagName) {
              case 'CODE':
                return <SyntaxHighlighter key={ index }
                                          ...
                                          style={ githubGist }>{ el.innerHTML }</SyntaxHighlighter>
              case 'A':
                return <a href={`${el.getAttribute('href')}`}
                          key={ index }
                          style={{ textDecoration: 'underline' }}>{ el.innerHTML }</a>
              case 'B':
          ...
          <div className="editor__buttons">
            <Button type="success" size="small" onClick={ handleSubmit }>Опубликовать</Button>
            <Button type="warning" size="small" onClick={ togglePreview }>Редактировать</Button>
          </div>
        </div>
        ...
        <div className="editor" style={{ display: preview ? 'none' : 'block' }}>
          ...
          <p className="text text--xs text--gray">Сформулируйте вопрос так, чтобы сразу было понятно, о чём речь.</p>
            <Input value={ title } onChange={ handleTitleChange } className="editor__title-input" />
          <h3 className="editor__title title--sm">Теги вопроса</h3>
            { !!tags.length &&
              <ul className="selected-tags">
                {
                  tags.map((tag, index) => {
                    return <li key={ index } className="selected-tags__item">{ tag.value }</li>
                  })
                }
              </ul>
            }
            <AutoComplete ref="tags"
          ...
          <p className="text text--xs text--gray">Опишите в подробностях свой вопрос, чтобы получить более точный ответ.</p>
          <div className="editor-panel">
            <Button.Group>
              <Select placeholder="Синтаксис" className="syntax-select"
              ...
              <Button type="primary" onClick={ _handleAddLink }><i className="fa fa-link" /></Button>
              <Button type="primary"><i className="fa fa-image" /></Button>
              <Button type="primary" onClick={ _handleAddBold }><i className="fa fa-bold" /></Button>
              <Button type="primary" onClick={ _handleAddBlockQuote }><i className="fa fa-quote-right" /></Button>
            </Button.Group>
          </div>
          ...
          <div className="editor__buttons">
            <Button type="warning" size="small" onClick={ togglePreview }>Предосмотр</Button>
          </div>
        </div>
      </div>
    )
